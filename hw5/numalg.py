...
def relaxed_poisson(phi0_args, rho, h, eps=1e-3):
    #TODO: make sure boundary condition phi=0 in infinity makes sense (forces me to take a large volume)
    ...
    phi0_mat, mask = phi0_args
    mask_indices = (mask == 1)
    phi0 = phi0_mat[mask_indices]
    ...
    n = 1
    while np.any(np.abs(phi_next - phi) >= eps):
        n += 1
        phi = phi_next
        phi_next = step(phi)
        phi_next[mask_indices] = phi0
